...
{ 
    ...
    public class Hid : object
    {
        ...
        /// <returns></returns>
        public HID_RETURN GetHIDDeviceList(UInt16 vID, UInt16 pID, out List<string> serialList)
        {
                ...
                for (int i = 0; i<deviceList.Count; i++)
                {
                    ...
                    if (device != INVALID_HANDLE_VALUE)
                    {
                        ...
                        if (attributes.VendorID == vID && attributes.ProductID == pID && deviceStr.Contains(serial))
                        {
                            ...
                            recvBuff = new byte[inputReportLength];
        ...
        public HID_RETURN Read(byte [] rbuff, int len)
        {
            if (deviceOpened)
            {
                try
